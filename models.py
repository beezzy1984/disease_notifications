...
ONLY_IF_ADMITTED = {'invisible': ~Eval('hospital_admission')}
ONLY_IF_LAB = {'invisible': ~Eval('specimen_taken')}
ONLY_IF_DEAD = {'invisible': ~Eval('deceased')}

REQD_IF_LAB = dict([('required', Eval('specimen_taken'))] + ONLY_IF_LAB.items())
...
class DiseaseNotification(ModelView, ModelSQL):
    ...
    symptoms = fields.One2Many('gnuhealth.disease_notification.symptom',
                               'name', 'Symptoms')
    date_onset = fields.Date('Date of Onset', states=RO_SAVED)
...
class TravelHistory(ModelView, ModelSQL):
    ...
    notification = fields.Many2One('gnuhealth.disease_notification', 'Notification')
